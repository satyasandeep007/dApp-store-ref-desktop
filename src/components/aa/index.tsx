...
function ReviewDialog(props) {
    ...
    const { createReview, txurl } = useReviewContext()
    ...
    const onSubmit = (evt) => {
        ...
        createReview( props.dappId, review.rating || 1, review.comment)
        console.log(result.isUpdating);
    }
...
function AppRatingList(props) {
    ...
    function getObjectByDappId(array, dappId) {
      return array.find(item => item.dappId === dappId);
  }
    // const data = getObjectByDappId(aa, props.id)

    const { wallet } = useSmartAccountContext(); 
    // @ts-ignore
    const { getReviews, provider, reviews, average, reviewCount } = useReviewContext()
    useEffect(() => {
      setAddress(wallet?.address)
      // @ts-ignore
      getReviews(props.id)
    },[provider])
    console.log({ reviews, average, reviewCount })
    return <>
        ...
        <Row className="gap-x-[18px] ">
            <p className="text-[24px] leading-[28px] font-[600]">{average}</p>
            <StarRating rating={average} />
        </Row>
        <small className="text-[14px] leading-[34px] font-[500] text-[#87868C]">{reviewCount} Ratings</small>
...
function DappList(props) {
    ...
    const data = getObjectByDappId(aa, query.id)
    ...
    const dApp: Dapp = data;


    const history = JSON.parse(localStorage.getItem('dApps') ?? "{}");
    localStorage.setItem('dApps', JSON.stringify(Object.assign({}, history, { [data.dappId]: dApp })));
