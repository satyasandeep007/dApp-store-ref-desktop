...
function averageRating(arr) {
  ...
  const getReviews = async(page:string) => {
    ...
    const formattedReviews = combineArrays(currentReviews[0], currentReviews[1])
  ...
  const createReview = async (dapp, raiting, content) => {
    ...
    try {
      ...
      const reviewContract = new ethers.Contract( contractAddress, abi, web3Provider);
      const reviewTx = await reviewContract.populateTransaction.addReview(dapp, raiting , content)
      const tx = {
        to: contractAddress,
        data: reviewTx.data
      }
      ...
      toast.success(<a href={`https://mumbai.polygonscan.com/tx/${txHash?.transactionHash}`} target='_blank' rel="noreferrer">Success! Click here for your transaction!</a>, {
        position: "top-right",
        autoClose: 15000,
        hideProgressBar: false,
        closeOnClick: true,
